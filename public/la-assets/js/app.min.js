...
$(document).ready(function() {
    ...
    function getLI(upload) {
        ...
        if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp"]) > -1) {
            image = '<img src="'+bsurl+'/files/'+upload.hash+'/'+upload.name+'?s=200">';
        } else {
    ...
    function loadFMFiles(page, keyword) {
        page = parseInt(page) || 1;
        keyword = keyword || '';
        // load uploaded files
        $.ajax({
            dataType: 'json',
            url: $('body').attr("bsurl")+"/"+adminRoute+"/uploaded_files?page=" + page + "&keyword=" + keyword,
    ...
    $(".load_more_files_listing").on("click", function() {
       ...
       var keyword = $('#files_listing_keyword').val()
       loadFMFiles(page, keyword);
    });

    $(".media_popup_show_files_listing").on("click", function() {
        $(this).closest('.media_popup_drop_zone').addClass('visible_files_listing');
        $(this).hide();
        $(this).closest('.media_popup_drop_zone').find('.media_popup_hide_files_listing').show();
        $("#files_listing_keyword").val('')
        loadFMFiles();
    });
    $(".media_popup_hide_files_listing").on("click", function() {
        $(this).closest('.media_popup_drop_zone').removeClass('visible_files_listing');
        $(this).hide();
        $(this).closest('.media_popup_drop_zone').find('.media_popup_show_files_listing').show();
    });
    ...
    fm_dropzone = new Dropzone("#fm_dropzone", {
        ...
        init: function() {
            ...
            this.on("success", function(file) {
                ...
                console.log(file);
                $(".media_popup_show_files_listing").trigger("click");
                $('#files_listing_keyword').val('');
    ...
    $("body").on("click", ".fm_file_sel", function() {
        ...
        if(type == "image") {
            ...
            $hinput.next("a").next(".uploaded_image").children("img").attr("src", bsurl+'/files/'+upload.hash+'/'+upload.name+"?s=200");
            $hinput.next("a").next(".uploaded_image").find('.uploaded_image_link').remove();
            $hinput.next("a").next(".uploaded_image").append('<div class="uploaded_image_link"><a href="' + bsurl+'/files/'+upload.hash+'/'+upload.name + '" target="_blank">Open Image in new window.</a></div>')
            ...
            $hinput.next("a").next(".uploaded_file").find('.uploaded_image_link').remove();
            $hinput.next("a").next(".uploaded_file").append('<div class="uploaded_image_link"><a href="' + bsurl+'/files/'+upload.hash+'/'+upload.name + '" target="_blank">Open file in new window.</a></div>')
        } else if(type == "files") {
            $hinput = $("input[name="+$("#image_selecter_origin").val()+"]");

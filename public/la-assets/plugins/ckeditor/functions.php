...
function accelerate_scripts_styles_method() {
	...
	*/
	...
	//}
  wp_enqueue_script( 'accelerate-navigation', ACCELERATE_JS_URL . '/responsiveslides.min.js', array( 'jquery' ), false, true );
  ...
  wp_enqueue_style( 'accordion-slider', get_template_directory_uri().'/css/accordion-slider.min.css', array(), '4.7.0' );
  wp_enqueue_style( 'accelerate-examples', get_template_directory_uri().'/css/examples.css', array(), '4.7.0' );
  
  wp_enqueue_script( 'accordionSlider', ACCELERATE_JS_URL.'/jquery.accordionSlider.min.js', array(), '3.7.3', false );
  wp_enqueue_script( 'staff', ACCELERATE_JS_URL.'/staff.js', array(), '3.7.4', false );
...
function accelerate_footer_copyright() {
	...
	$default_footer_value = sprintf( __( 'Всі права захищені &copy; %1$s %2$s.', 'accelerate' ), date( 'Y' ), $site_link );

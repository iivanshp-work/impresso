        ...
        <main class="dashboard">
            ...
            @if($userTransactions)
                ...
                @foreach($userTransactions as $userTransaction)
                    <div class="cards dashboard__card" data-id="{{$userTransaction->id}}" data-type="{{$userTransaction->type}}">
                                ...
                                <div class="dashboard__text">
                                    ...
                                    @elseif($userTransaction->type == 'validation_education')
                                        @if($userTransaction->education)
                                            <a href="javascript:void(0);">{{ str_limit($userTransaction->education->title, 15) }}</a>
                                    ...
                                    @elseif($userTransaction->type == 'validation_certificate')
                                        @if($userTransaction->certificate)
                                            <a href="javascript:void(0);">{{ str_limit($userTransaction->certificate->title, 15) }}</a>
                                    ...
                                    @elseif($userTransaction->type == 'validation_resume')
                                        @if($userTransaction->resume)
                                            <a href="javascript:void(0);">{{ str_limit($userTransaction->resume->title, 15) }}</a>
                                    ...
                                    @elseif($userTransaction->type == 'meetup_inviting')
                                        <a href="{{url('/profile/' . $userTransaction->meetup->user_id_invited)}}">{{$userTransaction->meetup->invitedUser ? ($userTransaction->meetup->invitedUser->name ? $userTransaction->meetup->invitedUser->name :$userTransaction->meetup->invitedUser->email) : ('Profile #' . $userTransaction->meetup->user_id_invited ) }}</a>
                                        {!!$userTransaction->meetup->invitedUser && $userTransaction->meetup->invitedUser->job_title ? '<span>' . $userTransaction->meetup->invitedUser->job_title . '</span>' : ''!!}
                                        <p>Meetup Invitation!</p>
                                    @elseif($userTransaction->type == 'meetup_accept')
                                        <a href="{{url('/profile/' . $userTransaction->meetup->user_id_inviting)}}">{{$userTransaction->meetup->invitingUser ? ($userTransaction->meetup->invitingUser->name ? $userTransaction->meetup->invitingUser->name :$userTransaction->meetup->invitingUser->email) : ( 'Profile #' . $userTransaction->meetup->user_id_inviting ) }}</a>
                                        <p>Meetup Accepted!</p>
                                    @elseif($userTransaction->type == 'meetup_declined')
                                        <a href="{{url('/profile/' . $userTransaction->meetup->user_id_invited)}}">{{$userTransaction->meetup->invitedUser ? ($userTransaction->meetup->invitedUser->name ? $userTransaction->meetup->invitedUser->name :$userTransaction->meetup->invitedUser->email) : ('Profile #' . $userTransaction->meetup->user_id_invited ) }}</a>
                                        <p>Meetup Declined</p>
                                    ...
                                    @elseif($userTransaction->type == 'referral_invited')
                                        <a href="javascript:void(0);">{{'IMPRESSO'}}</a>
                                        <p>Share: Referral Invited!</p>
                                    @elseif($userTransaction->type == 'referral_inviting')
                                        <a href="javascript:void(0);">{{'IMPRESSO'}}</a>
                                        <p>Share: Referral Inviting!</p>
                        ...
                        <div class="dashboard__bottom">
                            ...
                            @elseif($userTransaction->type == 'share')
                                <p>You have received through to share IMPRESSO.</p>
                            @elseif($userTransaction->type == 'validation_education')
                                @if($userTransaction->education)
                                    <p>You have used {{$userTransaction->amount}} XIMs for Education validation: {{$userTransaction->education->title}}</p>
                                @else
                                    <p>You have used {{$userTransaction->amount}} XIMs for Education validation</p>
                                @endif
                            @elseif($userTransaction->type == 'validation_certificate')
                                @if($userTransaction->certificate)
                                    <p>You have used {{$userTransaction->amount}} XIMs for Certificate validation: {{$userTransaction->certificate->title}}</p>
                                @else
                                    <p>You have used {{$userTransaction->amount}} XIMs for Certificate validation</p>
                                @endif
                            @elseif($userTransaction->type == 'validation_resume')
                                @if($userTransaction->resume)
                                    <p>You have used {{$userTransaction->amount}} XIMs for CV/Resume validation: {{$userTransaction->resume->title}}</p>
                                @else
                                    <p>You have used {{$userTransaction->amount}} XIMs for CV/Resume validation</p>
                                @endif
                            @elseif($userTransaction->type == 'meetup_inviting')
                            ...
                            @endif
                            <p class="bonus">@if($userTransaction->amount > 0)+@elseif($userTransaction->amount < 0)-@endif @if($userTransaction->amount >= 0){{$userTransaction->amount}} @else {{str_replace('-', '', (string)$userTransaction->amount)}} @endif {{getenv('CREDITS_LABEL')}}</p>
                        </div>
                    </div>
                @endforeach
            @else

...
<div class="box box-success">
	<div class="box-header" style="margin-bottom: 10px;">
		<div class="box-tools" >
			{!! Form::open(['action' => 'LA\PromosController@index', 'method' => 'get',  'id' => 'promos-search-form']) !!}
			@php
			...
			@if($keyword || $status)
				<div class="input-group">
					<a href="{{url(config('laraadmin.adminRoute') . "/promos")}}" type="submit" class="btn btn-default btn-sm" style="margin-left: 20px;">Clear</a>
				</div>
			@endif
		...
		<table id="example1" class="table table-bordered">
			...
			@if($values)
				@foreach($values as $value)
					<tr>
						@foreach( $listing_cols as $col )
							<td>@if ($col == $view_col || $col == 'tags'){!!$value->$col!!}@else{{$value->$col}}@endif</td>
						@endforeach
						<td>{!!$value->actions!!}</td>
					</tr>
				@endforeach
...
@push('styles')
	<style>
		...
		#promos-search-form .dropdown-field-search{
			width: 150px;
			display: flex;
			flex-direction: row;
			align-items: center;
		}
		#promos-search-form .dropdown-field-search select{
			margin: 0 15px;
		}
		#promos-search-form .field-search{
			width: 150px;
		}
	</style>
@endpush
